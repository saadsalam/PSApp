...
namespace AppWorks.UI.Controls.Extensions
{
    public static class Validator
    {
        ...
        public static bool IsValid(DependencyObject parent, bool isValidHiddenElement = false)
        {
            ...
            for (int i = 0; i != VisualTreeHelper.GetChildrenCount(parent); ++i)
            {
                ...
                if ((!isValidHiddenElement && (bool)child.GetValue(UIElement.IsVisibleProperty) && !IsValid(child))
                || (isValidHiddenElement && !IsValid(child)))
                {
                    valid = false;
                }
            }
